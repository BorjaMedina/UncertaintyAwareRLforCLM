        ...
        if len(smiles)>0:
            ...
            for i, ps in enumerate(preds):
                ...
                """if set_size == 1:
                    ...
                    uncertainty = abs(p_vals[i, 1] - p_vals[i, 0])
                    uncertainties.append(uncertainty)
                else:
                    uncertainties.append(self.model["significance"]) """
                confidence=np.sort(ps)
                uncertainties.append(1-confidence[-2])
 
 

        #if we want to make it compatible with the rest proabbly we should do: uncertainty=uncertainty/self.meanUnc simlar as its done in noisyLogP and noisyBertz    
        #if not we would need uncertainties to be passed as weight from 0 to 1 already

        #weights = [np.array(1 - np.array(uncertainties))]
        weights=[np.array(uncertainties)]
