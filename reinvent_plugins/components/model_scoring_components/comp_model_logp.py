...
class NoisyLogP:
    ...
    def __init__(self, params: Parameters):
        ...
        if "euclideanPCADesc" in self.uncertainty_type:
            ...
            self.combined_training_features = np.hstack([self.training_fingerprints, descriptors])
    ...
    def __call__(self, smiles: List[str]) -> ComponentResults:
        ...
        logger = logging.getLogger("reinvent")
        ## compute logp values
        logp_actual = [Crippen.MolLogP(Chem.MolFromSmiles(smile)) for smile in smiles]
        ...
        scores.append(
                np.array(
                    [
                        p+e if smi != "INVALID" else np.nan
                        for p, e, smi in zip(logp_actual, errors, smiles)
                    ]
                )
            )
        ...
        return ComponentResults(
            scores, uncertainty_type=self.uncertainty_type, metadata={"logp_actual": logp_actual, "errors": errors, "lengths": lengths}
        )
