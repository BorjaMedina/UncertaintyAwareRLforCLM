...
model_paths = glob("/projects/mai/se_mai/users/kcgd777_borja/uncertainty/06_Chemprop2Model/noLim/checkpoints_model*/best_*epoch=19*")
...
final_uncertainties = np.array(final_uncertainties, dtype=float)
scale = np.median(final_uncertainties) + 1e-8
final_dist = np.exp(-final_uncertainties / scale)
#final_uncertainties = np.clip(final_uncertainties / 10.0, 0.0, 1.0)
#final_dist=1-final_uncertainties

dataset = {"version": 1, "payload": {"predictions": list(final_dist), "uncertainties": list(final_uncertainties)}}
